...
def upload_document(body: UploadDocumentIn) -> CommonResult[UploadDocumentOut]:
    ...
    item = database.document_collection.insert_one(data.model_dump())
    return CommonResult.success(UploadDocumentOut(document_id=str(item.inserted_id)))
...
def get_document(doc_id: str = Query(description="文档id")) -> CommonResult[DocumentOut]:
    ...
    :return:
    """
    item = database.document_collection.find_one({"_id": ObjectId(doc_id)})
    data = DocumentOut.model_validate(item)
